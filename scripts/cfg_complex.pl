...
% Sentence
s([parse:s(NP,IV),depth:0,cond:no,sel:K]) -->
    !,
...
% Matrix negation
s([parse:s(NEG,S),depth:D1,cond:Cond,sel:K]) -->
    {D2 is D1 - 1},
    neg([parse:NEG,sel:K]),
    s([parse:S,depth:D2,cond:Cond,sel:K]).

% Conditional
s([parse:s(COND,S1,S2),depth:D1,cond:yes,sel:K]) -->
    {D2 is D1 - 1},
    cond([parse:COND,sel:K]),
    {K1 is K + 2},
    s([parse:S1,depth:D2,cond:no,sel:K1]),
    s([parse:S2,depth:D2,cond:no,sel:K1]).
...
% Negation
neg([parse:neg(Surf),sel:_]) -->
    {lex(neg,[surf:Surf])},
    Surf,
    {selector(1)}.

% Conditional
cond([parse:cond(Surf),sel:_]) -->
    {lex(cond,[surf:Surf])},
    Surf,
    {selector(1)}.
...
lex(neg,[surf:[it,is,not,the,case,that]]).
lex(cond,[surf:[if]]).
...
ptb(neg([X|List])) :-
    write('(NEG '), write(X), ptb(List).
ptb(cond([X|List])) :-
    write('(COND '), write(X), ptb(List).
ptb(punct([X|List])) :-
